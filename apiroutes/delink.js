...
router.patch('/dev/:devid/:locid', function (req, res) {
    ...
    location.update({ _id: req.params.locid }, { $pullAll: { devices: req.params.devid  } }, function (err, loca){
        if (err) throw err;
        ...
        if (err) throw err;
            ...
            lawndevices.findById(req.params.devid, function (err, dev1) {
                if (err) throw err;
                    ...
                    var found = false; i = 0;
                    //delete the device from location and save location 
                    for (var idval in loca.devices) {
                        test1 = JSON.stringify(loca.devices[i++]);
                        if (test1 === JSON.stringify(dev1.id)) { found = true; 
                            loca.devices.splice(i-1,1);
                            }
                    }
                    if (found) {
